...
webInterfaceGitUrl="https://github.com/MuhdSyafiq97/SamadGUI.git"
webInterfaceDir="${webroot}/admin"
piholeGitUrl="https://github.com/MuhdSyafiq97/SamadScript.git"
...
notify_package_updates_available() {
    ...
    if [[ -d "/lib/modules/$(uname -r)" ]]; then
        if [[ "${updatesToInstall}" -eq 0 ]]; then
            ...
            printf "  %b %bIt is recommended to update your OS after installing the SamAd!%b\\n\\n" "${INFO}" "${COL_LIGHT_GREEN}" "${COL_NC}"
        fi
    else
        printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
    ...
    # Otherwise,
        ...
        if is_command sudo ; then
            ...
            if [[ "$0" == "bash" ]]; then
                ...
                exec curl -sSL https://raw.githubusercontent.com/MuhdSyafiq97/SamAd/master/automated%20install/basic-install.sh | sudo bash "$@"
            else
